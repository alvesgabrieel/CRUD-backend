const express = require("express");
const app = express();
const handlebars = require("express-handlebars");
const bodyParser = require("body-parser");
const admin = require("./routes/admin");
const path = require("path"); //serve para manipular diretorios/pastas
const mongoose = require("mongoose");
const session = require("express-session");
const flash = require("connect-flash");
require("./models/Postagem");
const Postagem = mongoose.model("postagens");
require("./models/Categoria");
const Categoria = mongoose.model("categorias");
const users = require("./routes/usuario");
const passport = require("passport");
require("./config/auth")(passport);
const capitalizeFirstLetter = require("./helpers/firstLetter");

// >>> CONFIGURAÇÕES
//Sessão
app.use(
  session({
    secret: "cursodenode",
    resae: true,
    saveUninitialized: true,
  })
);

app.use(passport.initialize());
app.use(passport.session());

app.use(flash());

//Middlaware
app.use((req, res, next) => {
  //.locals cria variaveis globais
  res.locals.success_msg = req.flash("success_msg");
  res.locals.error_msg = req.flash("error_msg");
  res.locals.error = req.flash("error");
  res.locals.user = req.user || null;
  next();
});

//Body Parser
app.use(bodyParser.urlencoded({ extended: true }));
app.use(bodyParser.json());

//Handlebars
app.engine(
  "handlebars",
  handlebars.engine({
    helpers: {
      capitalizeFirstLetter: capitalizeFirstLetter,
    },
    defaultLayout: "main",
    runtimeOptions: {
      allowProtoPropertiesByDefault: true,
      allowProtoMethodsByDefault: true,
    },
  })
);
app.set("view engine", "handlebars");

//Mongoose
mongoose
  .connect("mongodb://127.0.0.1:27017/blogapp")
  .then(() => {
    console.log("Servidor MongoDB rodando com sucesso");
  })
  .catch((err) => {
    console.log(`erro ao se conectar ${err}`);
  });

//Public - arquivos estáticos - frontend
app.use(express.static(path.join(__dirname, "public")));

// > ROTAS

app.get("/", (req, res) => {
  Postagem.find()
    .populate("categoria")
    .sort({ data: "desc" })
    .then((postagens) => {
      res.render("index", { postagens: postagens });
    })
    .catch((err) => {
      req.flash("error_msg", "Houve um erro interno");
      res.redirect("/404");
    });
});

//Encontrar a postagem com base no slug
app.get("/postagem/:slug", (req, res) => {
  Postagem.findOne({ slug: req.params.slug })
    .then((postagem) => {
      if (postagem) {
        res.render("post/index", { postagem: postagem });
      } else {
        req.flash("error_msg", "Essa postagem não existe");
        res.redirect("/");
      }
    })
    .catch((err) => {
      req.flash("error_msg", "Houve um erro interno");
      res.redirect("/");
    });
});

app.get("/categorias", (req, res) => {
  Categoria.find()
    .then((categorias) => {
      res.render("categories/index", { categorias: categorias });
    })
    .catch((err) => {
      req.flash("error_msg", "houve um erro interno ao listar as categorias");
      res.redirect("/");
    });
});

//Encontrar a categoria com base no slug
app.get("/categorias/:slug", (req, res) => {
  Categoria.findOne({ slug: req.params.slug })
    .then((categoria) => {
      if (categoria) {
        Postagem.find({ categoria: categoria._id })
          .then((postagens) => {
            res.render("categories/posts", {
              postagens: postagens,
              categoria: categoria,
            });
          })
          .catch((err) => {
            req.flash("error_msg", "Houve um erro ao listar os posts");
            res.redirect("/");
          });
      } else {
        req.flash("error_msg", "Esta categoria não existe");
        res.redirect("/");
      }
    })
    .catch((err) => {
      req.flash(
        "error_msg",
        "Houve um erro interno ao carregar a página dessa categoria"
      );
      res.redirect("/");
    });
});

app.get("/404", (req, res) => {
  res.send("Erro 404");
});

app.use("/admin", admin);
app.use("/usuarios", users);

// > OUTROS
const PORT = 7070;
app.listen(PORT, () => {
  console.log(`servidor Node rodando em http://localhost:${PORT}`);
});
