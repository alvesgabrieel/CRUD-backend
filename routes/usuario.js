...
router.post("/registro", (req, res) => {
  ...
  if (req.body.senha != req.body.senha2) {
    erros.push({ texto: "senha não coincidem, tente novamente" });
  }

  if (erros.length > 0) {
    res.render("users/registro", { erros: erros });
  } else {
    //Condição caso ocorra todo preenchimento correto, os dados serao enviados ao BDD
    Usuario.findOne({ email: req.body.email })
            ...
            bcrypt.hash(novoUsuario.password, salt, (erro, hash) => {
              ...
              novoUsuario
                ...
                .catch((err) => {
                  req.flash(
                    "error_msg",
                    "Houve um erro ao criar um usuario, tente novamente!"
                  );
                  res.redirect("/usuarios/registro");
                });
            });
      ...
      .catch((err) => {
        req.flash("error_msg", "Houve um erro ao encontrar o usuario");
        res.redirect("/");
      });
  }
});
